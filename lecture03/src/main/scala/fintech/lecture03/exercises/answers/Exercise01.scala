...
object Exercise01 {
  ...
  final class Comparator[A] private (val compare: (A, A) => Int) {
    def contramap[B](f: B => A): Comparator[B] = new Comparator[B]((b1, b2) =>
      compare(f(b1), f(b2))
    )
  }

  object Comparator {
    val double: Comparator[Double] = new Comparator[Double]({(x1: Double, x2: Double) => x1.compareTo(x2)})
    val string: Comparator[String] = new Comparator[String](_.compareTo(_))
  }
