...
/*
Добавьте новый член AST дерева - IfAll
 */
object Exercise01CalculatorList extends App {
  sealed trait Expr[T]
  ...
  case class IfAll[T](iff: T => Boolean, cond: Expr[T], left: Expr[T], right: Expr[T]) extends Expr[T]
  ...
  class EvalListIntegral[T: Integral] {
    ...
    def calculate(expr: Expr[T]): List[T] = expr match {
      ...
      case IfAll(iff, cond, left, right) => ifff(iff, calculate(cond), calculate(right), calculate(left))
    }
  }
