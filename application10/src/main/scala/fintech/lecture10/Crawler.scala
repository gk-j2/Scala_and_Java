  ...
  private def handleMessage(state: CrawlerState, message: CrawlerMessage): (Task[Unit], CrawlerState) =
    message match {
      ...
      case req @ PageRequest(root, url, _) =>
        val effect =
          (for {
            html <- state.semaphore.withPermit(client.expect[String](url))
      ...
      case PageResponse(req, urls, mbResponse) =>
        ...
        val newState = state.copy(
          ...
          acc = mbResponse.fold(state.acc)(resp => (req.url, resp.size) :: state.acc)
        )
        (effect, newState)
    }

  val pattern = "href=[\"\']([^\"\']+)[\"\']".r
  private def extractUrls(root: String, html: String): List[String] =
    Try {
      (for {
